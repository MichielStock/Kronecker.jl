function _kron_mv_kernel_square!(q::AbstractArray{T, 1}, n::Int, i_left::Int, m::AbstractMatrix{T}, i_right::Int) where T<:Number
    ...
    if m != I # if matrix is the identity, skip matmul/div
        @inbounds for k in 1:i_left, i in 1:i_right
            slc = (((k-1) * n)*i_right + i): i_right :(((k * n) - 1)*i_right + i)
            @views q[slc] = m * q[slc]
        end
    end
    return q
...
function _kron_mv_kernel_square!(q::AbstractArray{T, 2}, n::Int, i_left::Int, m::AbstractMatrix{T}, i_right::Int) where T<:Number
    # apply kron(I(l), m, I(r)) where m is square to the given vector x, overwriting x in the process
...
# TODO: do something about the code duplication above^
